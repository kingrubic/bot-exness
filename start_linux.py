#!/usr/bin/env python3
"""One-click Ubuntu/Linux bootstrap: install missing pieces, then start web + MT5 + Wine Bridge."""
import os
import sys
import time
import shutil
...
os.environ.setdefault('WINEPREFIX', str(Path.home() / '.mt5'))
...
def ensure_venv():
    if VENV_PY.exists():
        return
    log('[1/6] Chưa có venv → đang tạo python3 -m venv venv ...')
...
def reexec_in_venv():
    ...
    os.execv(str(VENV_PY), [str(VENV_PY), str(ROOT / 'start_linux.py'), *sys.argv[1:]])
...
def ensure_mt5():
    log('[4/6] Kiểm tra / tự cài (nếu thiếu) / mở Exness MT5 (Wine) + Bridge 9999 ...')
    launcher = load_mt5_launcher()
    launcher.ensure_terminal_running()
    launcher.ensure_wine_bridge()
...
def main():
    os.chdir(ROOT)
    log('=' * 70)
    log('  EXNESS AUTO-TRADE — Ubuntu/Linux one-click start')
    log('  1 lệnh: cài cái còn thiếu, rồi start web + MT5 + Wine Bridge')
    log('=' * 70)
