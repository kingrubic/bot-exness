...
ready(() => {
  ...
  initSubListArrows();
  initHamburgerMenu();
  // initSidebarToggle() is handled exclusively by admin.js to avoid duplicate toggle events
  initSidebarTooltips();
...
function initThemeSwitcher() {
  ...
  // Remove any floating theme switcher widget to prevent blocking UI buttons
  document.querySelectorAll('.theme-switcher').forEach((el) => el.remove());
...
function initSidebarTooltips() {
  const links = $$('.menu-sidebar .navbar__list > li:not(.has-sub) > a');
  ...
  function show(link) {
    ...
    if (link.closest('.has-sub')) return;
    if (!link.dataset.label) return;
    const t = ensureTip();
